...
{
	//iSize is just the size of the current head
    int iSize = head->size;
	...
	//head now equals var plus the size we passed in
	//head's size is that initial size minus the size passed in plus the 16 byte header
    head = (void*) var + size;
    head->size = iSize - s;
...
void mem_manager_free(void* ptr)
{
	...
	else
	{
		...
		while(temp)
		{
			if(temp == fsl)
			{
				...
				//pointer after free is header now;
				fsl->next = header;
			}
			else
			{
	...
	while(head)
	{
		...
		if(p == (void*)headNext)
		{
			//get a new nexy variable and set it equal to head->next->next
...
{
	//if there's no head, then there's nothing to print
	if(!head)
	{
		printf("Invalid\n");
		return;
...
void* locate_split(mmfree_t* curr, int total_requested_size)
{
	...
	while(curr)
	{
		//if curr size is greater than the total requested size
		//make malloc pointer that equals size of curr + 16byte header
		//then size of that pointer equals total requested size minus its 16byte header(because we add 16bytes in mem man malloc)
		...
		//if the size of curr is already the total requested size
		//then just return null
		else if(curr->size == total_requested_size)
		{
			return curr;
		}
		//if curr is less than, then set curr its next pointer
		else
		{
			curr = curr->next;
		}
...
{
	//if the pointer that's being inserted is less than the header
	//return the pointer of what's being inserted
    if(to_insert < head)
    {
	...
	while(head)
	{
		...
		//if it is, we return head
		if(to_insert < head->next)
		{
			return (void*)head;
		}
		//of value after head is null, we return head
...
void init_mem(int free_space_size)
{
	...
	head->next = NULL;
	//print for how much space we passed in 
